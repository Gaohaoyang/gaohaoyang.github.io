...
|---|--|---|---------------|----------|---------|
| 基本功 | B | 20-30% |（1）编程题目（不敢写或写的不好的人动手不多），算法基础<br>（2）灵活多变，分别从广度和深度上扩充，不断拔高，直到不会。<br>
（3）观察编程功能+潜力，挤掉刷题党 | 经典题目：排序，树，图，文本处理 | 看个人的开发习惯<br>代码熟练程度<br>性能意识（时空复杂度）<br>代码风格（洁癖）|
...
| 意图| A | 5% | 目前拿到哪些offer，来面试的目的，侧重点（薪资、职位、地理位置等） | 你有什么要了解的 | 无 |
...
- 发散：
  - 预计会是什么样的词？（考业务敏感度+NLP功底）——标点符号、停用词，高频词，需要加tf-idf权重
