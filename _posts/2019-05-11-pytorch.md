...
就包含之前提到的network，dataset，train和evaluate。当然也包含一些笔者自己定制的模块，比如cal_metrics用来计算实验指标，cfg用来设置某些超参数，image_io_and_process用来对图像进行预处理和后处理，loss_functions用来定义与实现某些损失，utils用来承载一些其他操作，比如读写txt文档。
