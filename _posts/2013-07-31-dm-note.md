...
[Hadoop HDFS Commands with Examples](https://mindmajix.com/hadoop-hdfs-commands-with-examples)
...
hadoop fs -touch /usr/test.txt # 创建文件
hadoop fs -touchz /usr/test.txt # 创建空文件
# 文件信息
...
【2023-4-5】[参考](https://www.cnblogs.com/yejibigdata/p/6472500.html)
...
高级聚合主要有以下几种情况：
1. GROUPING SETS
  - 对同一个数据集的**多重**GROUP BY操作。事实上 GROUPING SETS是多个GROUP BY进行UNION ALL操作的简单表达，仅使用一个stage完成这些操作。GROUPING SETS 的子句中如果包换()数据集，则表示整体聚合。
  - 然而GROUPING SETS目前还有未解决的问题，参考[HIVE-6950](https://issues.apache.org/jira/browse/HIVE-6950)
2. ROLLUP和CUBE
...
- 都将分组中某列转为一个**数组**返回
- 不同：collect_list不去重而collect_set**去重**。
...
那么hive中怎么实现这个[功能](https://www.cnblogs.com/qi-yuan-008/p/13583959.html)呢？
...
数据集非常大时，找一个子集来加快数据分析。此时要数据采集工具以获得需要的子集。
...
- 使用 DISTRIBUTE 和 SORT 关键字来保证数据是随机分散到mapper和reducer的。
...
该方式允许Hive**随机**抽取N行数据，数据总量的**百分比**（n百分比）或**N字节**的数据。
...
【2022-11-19】[脉脉](https://maimai.cn/web/feed_detail?fid=1761062942&efid=U0XEC0nq1K5apnhvMNLUxg&share_channel=2&share_uid=3539&content_type=feed&use_rn=1&operation_id=dec05503-bf6f-4281-8d9a-463717901313) 不是贝壳找房的合作流程不够复杂，而是不够精准。 在这里，讨论这个是没用的。这事要改，需要更改贝壳分的体制…… 
