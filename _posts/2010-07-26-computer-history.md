...
能否通过图灵测试，是衡量机器智能程度的重要指标。这位“人工智能之父”过于乐观地预言：
...
`威诺格拉德模式挑战`（英语：Winograd Schema Challenge，缩写WSC）是多伦多大学计算机科学家`赫克托·莱韦斯克`提出的机器智能测试。该测试试图**改进传统的图灵测试**，通过向机器询问特别设计的选择题来检测其智能。
...
不同的 CPU 架构有不同的指令集，彼此不通用，这导致运行在上面的软件也不兼容，必须重新编译。
- 如果适配，一个架构下的软件就无法移植到另一个架构。

历史上有过多种 CPU 架构。目前最常见只剩下两种：`x86` 架构和 `ARM` 架构。
- x86 架构性能好，但是耗电多、电压高，主要用于**桌面电脑**和**服务器**
  - 生产厂商为 Intel 公司和 AMD 公司。
- ARM 架构耗电小、电压低，但是**单核性能**不如 x86，主要用于**移动设备**。
  - ARM 芯片的生产商有许多家，这是因为它的商业模式是**授权制**。英国的 ARM 公司出售指令集的授权，购买授权的公司可以基于公版的设计，开发自己的 ARM 芯片。高通、三星、华为、苹果等公司的芯片，都属于这个模式。
...
【2020-5-16】[苹果电脑为什么要换 CPU：Intel 与 ARM 的战争](https://www.toutiao.com/article/6838934273009910286)

iPad 的操作系统跟 iPhone 都基于 iOS。
- 如果 iOS 可以用于笔记本，就意味着可以跟桌面系统 MacOS 统一了。
- 如果 MacBook 和 iPhone 都用同一个操作系统，App 就能通用了。

2020年6月22日，WWDC（苹果全球开发者大会），苹果公司宣布更换 Mac 电脑的 CPU，从 Intel 公司的 `x86` 架构改成 `ARM` 架构。

苹果公司同时使用这两种架构的芯片，iPhone 和 iPad 的芯片是 ARM 架构，Mac 电脑的芯片是 x86 架构，这导致 iPhone 的 App 无法在 Mac 电脑运行。

Mac 电脑发生过两次 CPU 架构更改。
...
原因
- 一是 Intel 的 CPU 比 PowerPC 性能强，并且由于产量大，价格也便宜。
- 二是 Windows 电脑使用的是 x86 芯片，改了架构以后，Mac 电脑就也能安装 Windows，做到”一机双系统“。这可以促进当时处于市场劣势的 Mac 的销售。



近几年，`x86` 架构发展迟缓，`ARM` 架构则进步显著，已经从移动设备向桌面电脑和服务器进军了。
- Mac 电脑这次更换 CPU，就是准备从 `x86` 架构改为 `ARM` 架构。
...
用记事本编辑时，从文件读取的`UTF-8字符`被转换为`Unicode字符`到内存里，编辑完成后，保存的时候再把`Unicode`转换为`UTF-8`保存到文件：
...
|字符集|作用|位数|范围|
|---|---|---|---|
|GB2312|国家简体中文字符集，兼容ASCII|2个字节表示，能表示7445个符号，包括6763个汉字，几乎覆盖所有高频率汉字|高字节从A1-A7，低字节从A1到FE。将高字节和低字节分别加上0xA0即可得到编码。|
|GBK|GB2312的扩展，加入对繁体字的支持，兼容GB2312|2个字节表示，可表示21886个字符|高字节从81到FE，低字节从40到FE|
|GB18030|解决了中文、日文、朝鲜语等的编码，兼容GBK|变字节表示（1ASCII, 2,4字节）。可表示27484个文字|1字节从00到7F；2字节高字节从81到FE，低字节从40到7E和80到FE；4字节第一三字节从81到FE，第二四字节从30到39.|
...
- （2）GBK字符集
  - 作用：GB2312的扩展，加入对繁体字的支持，兼容GB2312.
...
中文编码主要有以下四种：
- `GB2312` 简体中文编码，一个汉字占用2个字节，在大陆是主要的编码方式。当文章/网页中包含繁体中文、日文、韩文等时，这些内容可能无法被正确编码。
- `BIG5` 繁体中文编码，主要在台湾地区使用。
...
【2023-3-28】Unicode 是全球文字统一编码。它把世界上的各种文字的每一个字符指定唯一编码，实现跨语种、跨平台的应用。
...
[参考](https://www.qqxiuzi.cn/zh/hanzi-unicode-bianma.php)

中文编码宽度
- GBK编码，一个汉字占2个字节。
- UTF-16编码，通常汉字占两个字节，CJKV扩展B区、扩展C区、扩展D区中的汉字占4个字节（一般字符的Unicode范围是U+0000至U+FFFF，而这些扩展部分的范围大于U+20000，因而要用两个UTF-16）。
- UTF-8编码是变长编码，通常汉字占3个字节，扩展B区以后的汉字占4个字节。

stevenliuyi [链接](https://www.zhihu.com/question/20451870/answer/15168034)

中文在utf-8中占 3 or 4个字节。utf8字符是变长字符，在这里补充一下uft-8的编码方式。
...
正则表达式识别中文
- [史上最全的正则表达式-匹配中英文、字母和数字](https://cloud.tencent.com/developer/article/1435804)
