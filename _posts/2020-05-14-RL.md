...
- Q-learning是强化学习中的一种，在Q-learning中，我们维护一张Q值表，表的维数为：状态数S * 动作数A，表中每个数代表在态s下可以采用动作a可以获得的未来收益的折现和——Q值。我们不断的迭代我们的Q值表使其最终收敛，然后根据Q值表我们就可以在每个状态下选取一个最优策略。由于这里主要介绍强化学习的在任务型对话中的应用，不进行详细的介绍，具体的可参照《[极简Qlearning教程](https://zhuanlan.zhihu.com/p/29213893)》
