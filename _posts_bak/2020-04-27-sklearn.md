...
- CountVectorizer()函数只考虑每个单词出现的频率（不考虑顺序），然后构成一个特征矩阵，每一行表示一个训练文本的词频统计结果。其思想是先根据所有训练文本，不考虑其出现顺序，只将训练文本中每个出现过的词汇单独视为一列特征，构成一个**词汇表**(vocabulary list)，该方法又称为**词袋法**(Bag of Words)。
...
# 问题：小问题，就是单字的词语，如“我”、“吞”、“呀”等词语在我们的词汇表中怎么都不见了呢？
